...
export const downloadResultTemplate = async (req, res, next) => {
  ...
  try {
    ...
    const scoreTypeResult = await pool
      .request()
      .input("CourseID", sql.Int, courseId)
      .query(
        `SELECT ScoreID FROM dbo.allocated_scoretype WHERE CourseID = @CourseID`,
      );

    if (scoreTypeResult.recordset.length === 0) {
      ca = 30;
      ex = 70;
    }

    const caID = scoreTypeResult.recordset[0].ScoreID;
    const exID = scoreTypeResult.recordset[0].ScoreID;

    ca = caID === 1 ? 30 : caID === 2 ? 40 : caID === 3 ? 50 : ca;
    ex = exID === 1 ? 70 : exID === 2 ? 60 : exID === 3 ? 50 : ex;
    ...
    //get active semester
    const activeSemesterResult = await pool.request().query(`
    ...
    if (!courseId) {
      return next(
        errorHandler(400, "Course, Session, and Semester are required"),
      );
    }
    ...
    const result = await pool
      ...
      .input("SemesterID", sql.Int, semesterID).query(`
                SELECT 
                    s.StudentID,
                    s.MatNo,
                    s.LastName,
    ...
    if (resultType === "Test") {
      ...
      worksheet.columns = [
        ...
        { header: `Test Score ${ca}`, key: "testScore", width: 15 },
      ...
      for (let i = 1; i <= 6; i++) {
        ...
        worksheet.getRow(i).getCell(2).font = {
          bold: true,
          color: { argb: "FF1E3A8A" },
        };
      }
      ...
      testScoreColumn.eachCell({ includeEmpty: false }, (cell, rowNumber) => {
        if (rowNumber > 7) {
          ...
          cell.dataValidation = {
            ...
            error: `Test score must be between 0 and ${ca}`,
          };
        }
      });
    ...
    worksheet.columns = [
      ...
      { header: `Exam Score (${ex})`, key: "examScore", width: 15 },
      // { header: 'Total Score (100)', key: 'totalScore', width: 15 },
      // { header: 'Grade', key: 'grade', width: 10 }
    ];
    ...
    examScoreColumn.eachCell({ includeEmpty: false }, (cell, rowNumber) => {
      if (rowNumber > 7) {
        cell.dataValidation = {
          ...
          showErrorMessage: true,
          errorTitle: "Invalid Score",
          error: `Exam score must be between 0 and ${ex}`,
        };
      }
    });
    ...
    return next(
      errorHandler(500, "Error generating template: " + error.message),
    );
  }
};
  ...
  try {
    ...
    if (ResultType === "Test") {
      worksheet.eachRow((row, rowNumber) => {
        ...
        const totalScore = totalScoreCell.result;
    ...
    // If it's an exam upload, we only expect the exam score column to be filled
    if (ResultType === "Exam") {
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber <= 7) return; // Skip header rows
        ...
        const matricNo = row.getCell(2).value;
        // const testScore = row.getCell(4).value;
        const examScore = row.getCell(4).value;
    ...
    if (results.length === 0) {
      return next(
        errorHandler(
          400,
          "No valid results found in the uploaded file. Please ensure at least one student has test or exam scores entered.",
        ),
      );
    }
    ...
    try {
      for (const result of results) {
        ...
        const studentResult = await transaction
          .request()
        ...
        const UploadSession = await transaction
          .request()
          .input("SessionName", sql.VarChar, sessionCell).query(`
                    SELECT SessionID FROM dbo.sessions WHERE SessionName = @SessionName
                `);

        const UploadSemester = await transaction
          .request()
          .input("SemesterName", sql.VarChar, semesterCell).query(`
        ...
        const existingResult = await transaction
          ...
          .input("SubmittedBy", sql.VarChar, lectid).query(`
        ...
        if (existingResult.recordset.length > 0) {
          ...
          if (
            existingResult.recordset[0].ResultType === "Exam" &&
            (existingResult.recordset[0].ResultStatus === "Submitted" ||
              existingResult.recordset[0].ResultStatus === "Approved")
          ) {
            return next(
              errorHandler(
                400,
                `Result for student ${MatricNo} cannot be edited. Contact HOD to unsubmit the results.`,
              ),
            );
          }
          ...
          const idandlevel = await transaction
            .request()
            .input("MatricNO", sql.VarChar, MatricNo).query(`
          ...
          await transaction
            ...
            .input("ResultType", sql.VarChar, ResultType)
            .input("lectid", sql.VarChar, lectid).query(`
...
export const getUploadedResults = async (req, res, next) => {
  ...
  try {
   ...
   `;
    ...
    return next(
      errorHandler(500, "Error fetching uploaded results: " + error.message),
    );
  }
};
...
export const updateScoreById = async (req, res, next) => {
  const lectid = req.user.id;
  const { MatricNo, CA_Score, Exam_Score, SessionID, SemesterID, CourseID } =
    req.body;

  console.log(CourseID, MatricNo, SessionID, SemesterID, CA_Score, Exam_Score);
  if (!lectid) {
    return next(
      errorHandler(403, "Lecturer ID and Matric Number are required"),
    );
  }
  ...
  try {
    ...
    //check score type and status before allowing update
    ...
    if (CA_Score > ca || Exam_Score > ex) {
      return next(
        errorHandler(
          400,
          `CA score cannot exceed ${ca} and Exam score cannot exceed ${ex}`,
        ),
      );
    }

    const checkType = await pool
      .request()
    ...
    if (
      checkType.recordset[0].ResultType === "Exam" &&
      (checkType.recordset[0].ResultStatus === "Submitted" ||
        checkType.recordset[0].ResultStatus === "Approved")
    ) {
      return next(
        errorHandler(
          400,
...
export const submitResultsToHOD = async (req, res, next) => {
  ...
  if (!ResultType) {
    return next(
      errorHandler(
        400,
        "ResultType query parameter is required and must be either 'Test' or 'Exam'",
      ),
    );
  }
  try {
    ...
    const assignmentCheck = await pool
      ...
      .input("CourseID", sql.Int, parseInt(courseID))
      .input("SessionID", sql.Int, parseInt(SessionID))
      .input("SemesterID", sql.Int, parseInt(SemesterID)).query(`
                SELECT 
                ca.AssignmentID,
                s.StaffId 
    ...
    if (assignmentCheck.recordset.length === 0) {
      return next(
        errorHandler(
          403,
          `${lectid}You are not assigned to this course for the selected session/semester`,
        ),
      );
    }
    ...
    if (result.rowsAffected[0] === 0) {
      return next(
        errorHandler(
          404,
          "No results found to submit or results have already been submitted/approved",
        ),
      );
    }
    ...
    return next(
      errorHandler(500, "Error submitting results: " + error.message),
    );
  }
};

export const ManualResultUpload = async (req, res, next) => {
  const lectid = req.user.id;
  const departmentid = req.user.departmentID;
  const errors = [];
  let successCount = 0;
  ...
  const { tableRow, CourseID, ResultType } = req.body;

  console.log(tableRow, CourseID, ResultType);

  if (!lectid) {
    return next(errorHandler(400, "Lecturer ID Required"));
  }
  ...
  try {
    ...
    try {
      for (const row of tableRow) {
        const student = await transaction
          .request()
          .input("MatricNo", sql.VarChar, row.Matric)
          .query(`SELECT MatNo, StudentID FROM Student WHERE MatNo =@MatricNo`);
        ...
        const MatricNo = student.recordset[0].MatNo;
        ...
        const checkResult = await transaction
          ...
          .query(`SELECT ResultID, ResultType, ResultStatus FROM results 
            WHERE MatricNo= @MatricNo
        ...
        if (checkResult.recordset.length > 0) {
          if (
            checkResult.recordset[0].ResultType === "Exam" &&
            (checkResult.recordset[0].ResultStatus === "Submitted" ||
              checkResult.recordset[0].ResultStatus === "Approved")
          ) {
            return next(
              errorHandler(
                400,
          ...
          if (ResultType === "Test") {
            ...
            let ca = 30;
            const scoreTypeResult = await transaction
              .request()
              .input("CourseID", sql.Int, CourseID)
              .query(
                `SELECT ScoreID FROM dbo.allocated_scoretype WHERE CourseID = @CourseID`,
            ...
            query = `UPDATE dbo.results
                ...
                SET CA_Score = @Score,
                            ...
                            WHERE ResultID = @ResultID  
                `;
            console.log("This is the query for test score update:", query);
          } else {
            ResultStatus = "Pending";
                ...
                SET Exam_Score = @Score,
                 ResultType = @ResultType,
                  ...
                  ResultStatus = @ResultStatus,
                            ...
                            WHERE ResultID = @ResultID  
                `;
            console.log("This is the query for exam score update:", query);
          }

          await transaction
            .request()
            .input("ResultType", sql.VarChar, ResultType)
            .input("Score", sql.Decimal(5, 2), parseFloat(row.Score))
            .input("lectid", sql.VarChar, lectid)
            .input("ResultID", sql.Int, checkResult.recordset[0].ResultID)
            .input("ResultStatus", sql.VarChar, ResultStatus)
            .query(query);
        } else {
          let ResultStatus = ``;
          let query = ``;
          const levlandId = await transaction
            .request()
            .input("MatNo", sql.VarChar, MatricNo)
            .query(
              `SELECT StudentID, LevelID FROM dbo.Student WHERE MatNo = @MatNo `,
            );

          if (levlandId.recordset[0].length === 0) {
            return next(
              errorHandler(
                400,
                `Student with Matric Number ${MatricNo} not found`,
              ),
            );
          }

          const LevelID = levlandId.recordset[0].LevelID;
          const StudentID = levlandId.recordset[0].StudentID;

          const courseDat = await transaction
            .request()
            .input("CourseID", sql.Int, CourseID)
            .query(
              `SELECT  credit_unit FROM dbo.courses WHERE  course_id = @CourseID  `,
            );

          if (courseDat.recordset[0].credit_unit === 0) {
            return next(errorHandler(400, "Wrong course Id"));
          }

          const CreditUints = courseDat.recordset[0].credit_unit;
          ...
          if (ResultType === "Test") {
            ...
            query = `
                            INSERT INTO dbo.results 
                            (MatricNo, CourseID, SessionID, SemesterID, CA_Score, CreditUnits, StudentID, LevelID, ResultType,ResultStatus, SubmittedDate, SubmittedBy)
                            VALUES 
                            (@MatricNo, @CourseID, @SessionID, @SemesterID, @Score, @CreditUnits, @StudentID, @LevelID, @ResultType, @ResultStatus, GETDATE(), @lectid)
                        `;
          } else {
            ResultStatus = "Pending";
              let ex = 70;
            ...
            query = `
                            INSERT INTO dbo.results 
                            (MatricNo, CourseID, SessionID, SemesterID, Exam_Score, CreditUnits, StudentID, LevelID, ResultType,ResultStatus, SubmittedDate, SubmittedBy)
                            VALUES 
                            (@MatricNo, @CourseID, @SessionID, @SemesterID, @Score, @CreditUnits, @StudentID, @LevelID, @ResultType, @ResultStatus, GETDATE(), @lectid)
                        `;
          }
          ...
          await transaction
            ...
            .input("SemesterID", sql.Int, SemesterID)
            .input("Score", sql.Decimal(5, 2), parseFloat(row.Score))
            .input("CreditUnits", sql.Int, CreditUints)
            .input("StudentID", sql.Int, StudentID)
            .input("LevelID", sql.Int, LevelID)
            .input("ResultType", sql.VarChar, ResultType)
            .input("ResultStatus", sql.VarChar, ResultStatus)
      ...
      res.status(200).json({
        ...
        message: "Results uploaded successfully",
        summary: {
          total: tableRow.length + errorCount,
          successful: successCount,
          failed: errorCount,
        },
        errors: errors.length > 0 ? errors : undefined,
    ...
    console.log("Error uploading manually: ", error.stack);
    return next(errorHandler(500, "Error Uploading Result"));
  }
};


export const getResultUploadTemplate = async (req, res, next) => {
  const {courseId, resultType} = req.query;
    const departmentid = req.user.departmentID;

console.log("CourseID:", courseId, "ResultType:", resultType, "DepartmentID:", departmentid);
   try {
    const pool = await poolPromise;
    ...
    const result = await pool
      ...
      .input("SemesterID", sql.Int, semesterID).query(`
                ...
                FROM dbo.registered_courses cr
                INNER JOIN dbo.Student s ON cr.mat_no = s.MatNo
                INNER JOIN dbo.courses c ON c.course_id = @CourseID
...
// INNER JOIN dbo.Student s ON cr.mat_no = s.MatNo
//                 INNER JOIN dbo.courses c ON c.course_id = @CourseID
//                 LEFT JOIN dbo.sessions ses ON cr.session_id = ses.SessionID
//                 LEFT JOIN dbo.semesters sem ON c.semester = sem.SemesterID
//                 LEFT JOIN dbo.AppDepartment d ON s.department = d.DepartmentID
//                 LEFT JOIN dbo.levels l ON s.LevelID = l.LevelID
//                 WHERE cr.course_id = @CourseID
...
//                     AND cr.department_id = @departmentid
    ...
    console.log("Error fetching template data:", error.message);
    return next(
      errorHandler(500, "Error fetching template data: " + error.message),
    );
  }
