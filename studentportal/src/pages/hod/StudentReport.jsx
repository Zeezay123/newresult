...
const StudentReport = () => {
  ...
  React.useEffect(() => {
    if (selectedProgramme && selectedLevel) {
      fetchStudentReport(selectedProgramme, selectedLevel)
    }
  ...
  const fetchProgrammesAndLevels = async () => {
    try {
      ...
      const response = await fetch('/api/hod/results/programmes-levels/', {
        credentials: 'include'
      })
      ...
      console.error('Error loading report filters:', fetchError)
  ...
  const fetchStudentReport = async (programmeId, levelId) => {
    try {
      setLoading(true)
      setError('')

      const response = await fetch('/api/hod/results/semester-summary', {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      console.error('Error loading student report:', fetchError)
      setStudents([])
      ...
      <div className='rounded-lg bg-white p-5 shadow-sm'>
        <div className='flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between'>
          <div>
            ...
            <p className='text-sm text-slate-600'>View probation and best-student reports for the selected programme and level.</p>
          </div>
        ...
        {(selectedProgrammeName || selectedLevelName) && (
          <div className='mt-4 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700'>
            Showing report for {selectedProgrammeName || 'Selected Programme'} {selectedLevelName ? `, ${selectedLevelName}` : ''}
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className='rounded-lg bg-white shadow-sm'>
            <div className='border-b border-slate-200 px-4 py-4'>
              ...
              <p className='mt-1 text-sm text-slate-600'>Student with the highest CGPA for the selected programme and level.</p>
            </div>
