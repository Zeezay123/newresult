...
import { Filter, Download, CheckCircle, Loader2, ChevronDown, ChevronUp } from 'lucide-react'

const DeanLevelResults = () => {
  ...
  React.useEffect(() => {
    if (selectedDepartment && selectedProgramme && selectedLevel) {
      fetchAllResults(selectedDepartment, selectedProgramme, selectedLevel)
    }
  ...
  const fetchFilters = async () => {
    try {
      ...
      if (fetchedDepartments.length > 0) {
        setSelectedDepartment(String(fetchedDepartments[0].DepartmentID))
      }
  ...
  const fetchAllResults = async (
    ...
    try {
      ...
      const buildRequestOptions = () => ({
        ...
        credentials: 'include',
        body: JSON.stringify(getRequestBody(departmentId, programmeId, levelId))
      })

      const [prevCumRes, currentCoursesRes, summaryRes, carryoversRes] = await Promise.all([
        fetch('/api/dean/results/previous-cumulative', buildRequestOptions()),
        fetch('/api/dean/results/current-courses', buildRequestOptions()),
        fetch('/api/dean/results/semester-summary', buildRequestOptions()),
        fetch('/api/dean/results/carryovers', buildRequestOptions())
      ])
  ...
  const handleDownloadResults = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/dean/results/download-level-results?${params.toString()}`, {
        method: 'GET',
        credentials: 'include'
      })
      ...
      let filename = 'Dean_Level_Results.xlsx'
      ...
      console.error('Error downloading dean results:', error)
  ...
  const handleApproveResults = async () => {
    if (!selectedDepartment || !selectedProgramme || !selectedLevel) return
    if (!window.confirm('Approve these HOD-approved level results for faculty approval?')) {
      return
    }

    setActionState('approve')
    try {
      const response = await fetch('/api/dean/results/approve-level-results', {
        method: 'PUT',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  const selectedDepartmentName = departments.find((item) => String(item.DepartmentID) === selectedDepartment)?.DepartmentName
  const selectedProgrammeName = programmes.find((item) => String(item.ProgrammeID) === selectedProgramme)?.ProgrammeName
  const selectedLevelName = levels.find((item) => String(item.LevelID) === selectedLevel)?.LevelName
      ...
      <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
        <div className='py-2'>
          ...
          <p className='text-sm text-slate-500'>
            Dean review of HOD-approved level results at faculty scope
          ...
          {selectedDepartmentName && selectedProgrammeName && selectedLevelName && (
            <p className='text-sm text-slate-600 mt-1'>Currently viewing {selectedDepartmentName} - {selectedProgrammeName} - {selectedLevelName}</p>
          )}
        </div>
      ...
      {loading ? (
        ...
        <>
          <section className='bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-900'>
            These results have been approved at HOD level and are awaiting final dean decision.
          </section>
                  ...
                  <table className='w-full text-sm'>
                    ...
                    <tbody>
                      {previousCumulative.length === 0 ? (
                        <tr>
                          <td colSpan='8' className='px-4 py-8 text-center text-gray-500'>No previous cumulative data available for this department, programme, and level</td>
                        </tr>
                      ) : (
          ...
          <section className='bg-white rounded-lg shadow-sm'>
            <div className='p-4 border-b border-gray-200 flex items-center justify-between cursor-pointer hover:bg-gray-50' onClick={() => setIsTable2Collapsed(!isTable2Collapsed)}>
              <div>
                ...
                <p className='text-sm text-gray-500'>All approved exam results for the selected department, programme, and level</p>
              </div>
              <button className='p-2'>
                        ...
                        <tr>
                          ...
                          {(() => {
                            ...
                            return Array.from(allCourses.values()).map((course) => (
                              <th key={course.CourseCode} className='px-3 py-2 text-center font-semibold border border-gray-300 min-w-20'>
                    ...
                    <table className='w-full text-sm border-collapse'>
                      ...
                      <tbody>
                        {paginate(carryovers, currentPage4).map((student, idx) => {
                          const carryoverCodes = [
                            ...student.failedCourses.map((course) => course.CourseCode),
                            ...student.missedCourses.map((course) => course.CourseCode)
                          ]
                          ...
                          return (
                            <tr key={student.MatricNo} className='border-b hover:bg-gray-50'>
                              ...
                              <td className='px-4 py-3 border'>
                                <span className='font-mono text-red-600 font-semibold'>{carryoverCodes.join(', ')}</span>
                              </td>
                            </tr>
                          )
                        })}
...
export default DeanLevelResults