...
const ManualUpload = ({onClose, selectedCourse, resultType, courses}) => {
  ...
  console.log('succ modal',succModal)
    ...
    const newItem = {
      ...
      console.log("ManualUpload Props:", { selectedCourse, resultType, courses });

      let courseSelect = courses.filter(course => course.course_id == selectedCourse);

   
    const handleAddRow = () => {   
        setTableRow(prevRow => [...prevRow, newItem]
        );
     }
 
     const handleRemoveRow=(id)=>{

       const newTableRow =  tableRow.filter(row=> row.id !== id )
...
const getResultDesgin = (resultType) =>{
  ...
  return (
    ...
    <Modal  show={true}  popup={true} dismissible onClose={handleOnClose}>
    <ModalHeader>
        ...
        <table className='w-full'>
            ...
            <tbody> 
                {   tableRow.map((row, index) => (
                    <tr key={row.id} >
                        <td className='p-2 '> <TextInput  type='text' value={row.Matric} onChange={(e)=>setTableRow(prev=> prev.map(item => row.id == item.id ? {...item, Matric: e.target.value} : item) )}   /> </td>
